...
#else
  uint64_t rhi, rlo;
  __asm__("mulq %[B]" : "=d"(rhi), "=a"(rlo) : "a"(a), [B] "rm"(b) : "cc");
  *h = rhi;
  return rlo;
#endif
}
...
#if defined(__ADX__)
#define my_addcarry_u64 _addcarryx_u64
#define my_subborrow_u64 _subborrowx_u64
#else
#define my_addcarry_u64 _addcarry_u64
#define my_subborrow_u64 _subborrow_u64
#endif
...
static void inline imm_mul(uint64_t *x, uint64_t y, uint64_t *dst, uint64_t *carryH) {
  ...
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[1], y, &h), carry, (unsigned long long *)(dst + 1));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[2], y, &h), carry, (unsigned long long *)(dst + 2));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[3], y, &h), carry, (unsigned long long *)(dst + 3));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[4], y, &h), carry, (unsigned long long *)(dst + 4));
  carry = h;
#if NB64BLOCK > 5
  c = my_addcarry_u64(c, _umul128(x[5], y, &h), carry, (unsigned long long *)(dst + 5));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[6], y, &h), carry, (unsigned long long *)(dst + 6));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[7], y, &h), carry, (unsigned long long *)(dst + 7));
  carry = h;
  c = my_addcarry_u64(c, _umul128(x[8], y, &h), carry, (unsigned long long *)(dst + 8));
  carry = h;
#endif
  *carryH = carry;
}
...
static void inline imm_imul(uint64_t *x, uint64_t y, uint64_t *dst, uint64_t *carryH) {
  ...
  carry = h;
#endif
  c = my_addcarry_u64(c, _mul128(x[NB64BLOCK - 1], y, (int64_t *)&h), carry,
                      (unsigned long long *)(dst + NB64BLOCK - 1));
  carry = h;
  *carryH = carry;
}
...
static void inline imm_umul(uint64_t *x, uint64_t y, uint64_t *dst) {
  ...
  carry = h;
#endif
  my_addcarry_u64(c, 0ULL, carry, (unsigned long long *)(dst + (NB64BLOCK - 1)));
}
